...
function collectCspViolations(page: Page): string[] {
  ...
  page.on('console', (msg) => {
    if (/violates Content Security Policy|Refused to (load|execute|connect|apply)/i.test(msg.text())) {
...
test.describe('CSP smoke tests', () => {
  ...
  test('Turnstile challenge script loads under CSP', async ({ page }) => {
    ...
    const probe = await fetch('https://challenges.cloudflare.com/turnstile/v0/api.js')
      .then((r) => r.ok)
      .catch(() => false);
    test.skip(!probe, 'challenges.cloudflare.com unreachable (offline environment?)');

    await expect
      .poll(() => page.evaluate(() => Boolean((window as { turnstile?: unknown }).turnstile)), {
        timeout: 15_000,
      })
      .toBe(true);
  ...
  test('WASM simulator runs a circuit under CSP', async ({ page }) => {
    ...
    await expect(page.locator('.state-entry').first()).toBeVisible({ timeout: 15_000 });
