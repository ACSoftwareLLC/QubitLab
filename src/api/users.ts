...
export async function getAdminActions(userId: string): Promise<AdminAction[]> {
  const res = await fetch(
    `/auth/admin/users/${encodeURIComponent(userId)}/actions`,
    {
      credentials: "include",
    },
  );
...
export async function banUser(
  ...
  const res = await fetch(
    `/auth/admin/users/${encodeURIComponent(userId)}/ban`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
...
export async function unbanUser(
  ...
  const res = await fetch(
    `/auth/admin/users/${encodeURIComponent(userId)}/unban`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
