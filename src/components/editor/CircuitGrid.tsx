...
export function CircuitGrid({
    ...
    const displayOp = (op: PlacedOp): PlacedOp => {
        ...
        if (slotPreview && slotPreview.opId === op.id) {
            const key =
                slotPreview.slot.kind === "target" ? "targets" : "controls";
            const idx = slotPreview.slot.index;
            next = {
                ...next,
                [key]: next[key].map((w, i) =>
                    i === idx ? slotPreview.wire : w,
                ),
            };
        }
        return next;
            ...
            >
                ...
                {Array.from({ length: doc.numBits }, (_, w) => {
                    ...
                    const probRightX =
                        width -
                        GRID.padRight -
                        (measured != null ? PROB_CLASSICAL_GAP : 0);
                    return (
                        <g key={`wire-${w}`} className="ev2-wire-group">
                            ...
                            {measured != null && (
                                <g
                                    transform={`translate(${width - GRID.padRight - 10}, ${y})`}
                                >
                                    <rect
                                        x={-8}
                                        y={-9}
                ...
                {doc.ops.map((op) => (
                    <g
                        ...
                        className={`ev2-op${selectedIds.has(op.id) ? " selected" : ""}${
                            movePreview?.opId === op.id ||
                            slotPreview?.opId === op.id
                                ? " dragging"
                                : ""
            ...
            {doc.ops.length === 0 && !ghost && (
                <div className="ev2-empty-hint">
                    ...
                    <span className="ev2-empty-sub">
                        Drag a gate from the palette onto a wire, or click a
                        gate then click a cell. Columns run left → right in
                        time.
                    </span>
                    <span className="ev2-empty-examples-label">
                        Or start from an example
                    </span>
                    <div className="ev2-example-row">
                        {EXAMPLE_CIRCUITS.map(
                            ({ key, title, blurb, circuit }) => (
                                <button
                                    key={key}
                                    type="button"
