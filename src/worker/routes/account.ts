...
account.patch("/username", async (c) => {
  ...
  const overLimit = await checkUserActionLimit(
    c,
    "username_change",
    3,
    HOUR_MS,
...
account.patch(
  "/password",
  rateLimitUser("password_change", 10, 15 * 60 * 1000),
  async (c) => {
    const body = await c.req.json();
    ...
    const currentSessionHash = currentSessionId
      ? await hashSessionId(currentSessionId)
      : null;
    await runQuery(c, `DELETE FROM sessions WHERE user_id = ? AND id <> ?`, [
      user.id,
      currentSessionHash ?? "",
    ]);
...
account.post("/avatar", async (c) => {
  ...
  if (!sniffed || sniffed !== ext) {
    return jsonError(
      c,
      "Avatar file content does not match its extension",
      400,
    );
  }
  ...
  if (user.pfp_key) {
    c.executionCtx.waitUntil(
      r2Delete(c, "AVATARS", user.pfp_key).catch(() => {}),
    );
  }
